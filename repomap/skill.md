...
```
/repomap                  # 无路径 → 强制确认目标
/repomap <path>           # 整理指定目录（相对于工作目录）
/repomap <path> -p        # preview 模式，先确认再执行
/repomap root             # 整理整个工作目录
```
...
### 路径解析
- 相对路径基于当前工作目录解析
- `/repomap tech` → 找工作目录下第一个名为 `tech` 的目录
- 多个匹配时选最浅层级

### 确认机制
- `/repomap` 无参数 → 强制询问目标目录
- `/repomap root` 在非项目目录 → 强制确认
- `/repomap <path>` → 直接执行
- `-p` 参数 → 预览后确认
